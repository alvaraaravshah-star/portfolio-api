async function loadOptions(){
  try{
    const d = await fetch('/available-dates');
  ...
  try{
    const r = await fetch('/available-investor-types');
...
async function getRecommendation(){
  ...
  try{
    const resp = await fetch('/recommend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target_date:date,investor_type:investor})});
